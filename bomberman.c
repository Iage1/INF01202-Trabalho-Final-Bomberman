...
void moveInimigos(Enemy inimigo[], char mapa[][COLUNAS], Player *jogador)
{
    ...
    for (int i = 0; i < NINIMIGOS; i++)
    {
        ...
        {
            if (rand() % 5 == 0 )//20% de chance de mudar a dire��o sempre que o inimigo estiver alinhado com a matriz.
            {
        ...
        case 0:
            ...
            {
                inimigo[i].direcao = rand() % 4; // Troca a dire��o se n�o for poss�vel
            }
            break;
        case 1:
...
//Fun��o principal para manipular bombas
void plantaBomba(Player *jogador, char mapa[][COLUNAS], Enemy inimigo[], Bomb bomba[])
{
    if (IsKeyPressed(KEY_B))
    {
        ...
        {
            if (bomba[k].estadoAtiva == 0)   //Percorre as bombas at� encontrar uma inativa
            {
    ...
    for (int k = 0; k < MAXBOMBA; k++)
    {
        if (bomba[k].estadoAtiva == 1)  //Percorre as bombas at� achar uma ativa
        {
...
void propagaExplosao(char mapa[][COLUNAS], Enemy inimigo[], Bomb bomba[], int x, int y, int direcx, int direcy, Player *jogador)
{
    for (int z = 0; z < MAXBOMBA; z++)   //Bomba com alcance de 100 pixels -> 3 pixels pra todas as direcoes (incluindo o do meio)
    {
        ...
        for (int i = 0; i < NINIMIGOS; i++)
        {
            ...
            if (CheckCollisionRecs(retanguloExplosao, retanguloInimigo) == 1) //Checa se h� colisao entre dois retangulos. Funcao bool.
            {
...
void desenhaExplosao(Bomb bomba[], char mapa[][COLUNAS])
{
    for (int k = 0; k < MAXBOMBA; k++)
    {
        if (bomba[k].estadoExplosao == 1)
        {
            ...
            // Propaga o desenho da explos�o em todas as dire��es
...
void menu(int *pausa, char mapa[][COLUNAS], Player *jogador, Enemy inimigo[], Bomb bomba[], char *nome_arq, Save *optimus)
{
    ...
    DrawText("Voltar (V)", 402, 270, TAMTEXTO, WHITE);
        ...
        switch(opcao){
        case 'N':
            novoJogo(nome_arq, mapa, jogador, inimigo, bomba, pausa);// Reseta todas as informa��es para como est�o no arquivo, ou seja, como eram originalmente.
            *pausa = 0;
            break;
        case 'C':
...
//Salva o jogo em um arquivo bin�rio, a partir da struct SAVE
...
void loadGame(char *nome_arq, char mapa[][COLUNAS], Player *jogador, Enemy inimigo[], Bomb bomba[], int *pausa, Save *optimus)
{
    ...
    *pausa = 1;
    ...
    // Abrir arquivo para leitura bin�ria
    ...
    // Restaurar os dados nas vari�veis do jogo (campo por campo)
    for (int i = 0; i < LINHAS; i++) {
        for (int j = 0; j < COLUNAS; j++) {
